...
2. ### Train an ANN to represent F(**s**)
   ...
   - It will plot "Training loss" and "Validation loss". Ensure that they are in reasonable limits (close to zero).
   - It will also generate "predicted free energy" surface plot. Ensure that it is correct.
...
4. ### Perform MD with "IMetaD".
   ...
   - Note down the "simualtion time". Multiply is with the "acceleration factor" to recover "unbiased simulation time".
5. #### Statistics 
   - Perform multiple simulations with the same initial structure but different velocities.
   - Run "python cdf.py". It will compute the _empirical_ cumulative distribution function (ECDF) from the computed unbiased transition times and fit to Theretical CDF (TCDF) to estimate $\tau$.
